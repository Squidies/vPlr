<template lang="pug">
  #audio(ref="vPlr")
    plr-display(
      ...
      v-bind:displaytimeremaining="displayTimeRemaining"
      v-on:displaycurrenttimechange="_displayCurrentTimeChange"
      v-on:changecurrenttime="_changeCurrentTime"
      v-on:toggleremainingdur="_toggleRemainingDuration"
    )
    plr-controls(
...
export default {
  ...
  data () {
    return {
      ...
      filesLoaded: false,
      displayTimeRemaining: localStorage.getItem('vPlr_remaining-duration') || true,
      playlist: [],
      index: -1,
